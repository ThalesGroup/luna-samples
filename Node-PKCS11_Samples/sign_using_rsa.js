 ...
 * - Signing is performed using SHA512_RSA_PKCS mechanism.
 */
...
if (process.argv.length !== 3) {
  usageAndExit([
    "Usage:",
    "node sign_using_rsa.js <slot_label>",
    "",
    "Example:",
    "node sign_using_rsa.js myPartition\n",
  ]);
}
...
(async () => {
  ...
  await withSession(slotLabel, async (session) => {
    ...
    const sign = session.createSign(graphene.MechanismEnum.SHA512_RSA_PKCS, keys.privateKey);
    const signature = sign.once(Buffer.from(plaintext, "utf8"));
    console.log("Plaintext signed.");

    const verify = session.createVerify(graphene.MechanismEnum.SHA512_RSA_PKCS, keys.publicKey);
