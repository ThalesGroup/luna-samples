 ...
 * - Signing is performed using RSA_PKCS mechanism.
 */

"use strict";
const { graphene, withSession, usageAndExit, getPlaintext, toHex } = require("./lib/helper");

console.log("\nsign_using_rsa_pkcs1.js\n");

if (process.argv.length !== 3) {
  usageAndExit([
    "Usage:",
    "node sign_using_rsa_pkcs1.js <slot_label>",
    "",
    "Example:",
    "node sign_using_rsa_pkcs1.js myPartition\n",
  ]);
}
...
(async () => {
  ...
  await withSession(slotLabel, async (session) => {
    ...
    const sign = session.createSign(graphene.MechanismEnum.RSA_PKCS, keys.privateKey);
    const signature = sign.once(Buffer.from(plaintext, "utf8"));
    console.log("Plaintext signed.");

    const verify = session.createVerify(graphene.MechanismEnum.RSA_PKCS, keys.publicKey);
