 ...
 * - Text is signed using ECDSA_SHA512 mechanism.
 */

"use strict";
const { graphene, withSession, usageAndExit, getPlaintext, toHex } = require("./lib/helper");

console.log("\nsign_using_ecdsa_sha512.js\n");

if (process.argv.length !== 3) {
  usageAndExit([
    "Usage:",
    "node sign_using_ecdsa_sha512.js <slot_label>",
    "",
    "Example:",
    "node sign_using_ecdsa_sha512.js myPartition\n",
  ]);
}

const slotLabel = process.argv[2];
const curveId = "secp521r1";
...
(async () => {
  ...
  await withSession(slotLabel, async (session) => {
    ...
    const sign = session.createSign(graphene.MechanismEnum.ECDSA_SHA512, keys.privateKey);
    const signature = sign.once(Buffer.from(plaintext, "utf8"));
    console.log("Plaintext signed.");

    const verify = session.createVerify(graphene.MechanismEnum.ECDSA_SHA512, keys.publicKey);
