 ...
 * OBJECTIVE:
 * - Wrap/unwrap a secret key with DES3_CBC_PAD (mirrors JSP WrapUnwrapSecretKeyUsing_DES3_CBC_PAD).
 */

"use strict";
const { graphene, withSession, usageAndExit, toHex } = require("./lib/helper");

console.log("\nwrap_secret_key_using_des3_cbc_pad.js\n");
if (process.argv.length !== 3) {
  usageAndExit([
    "Usage:",
    "node wrap_secret_key_using_des3_cbc_pad.js <slot_label>",
    "",
    "Example:",
    "node wrap_secret_key_using_des3_cbc_pad.js myPartition\n",
  ]);
}
const slotLabel = process.argv[2];
(async () => {
  await withSession(slotLabel, async (session) => {
    const wrappingKey = session.generateKey(graphene.MechanismEnum.DES3_KEY_GEN, {
    ...
    console.log("DES3 wrapping key + AES data key generated.");
    const iv = Buffer.alloc(8, 0x22);
    const alg = {
    ...
    console.log("Key wrapped with DES3_CBC_PAD.");
    ...
    console.log("  --> sample hex    :", toHex(wrapped).slice(0, 32) + "...");
    console.log();
  });
})();
