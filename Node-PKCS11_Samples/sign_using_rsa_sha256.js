 ...
 * - Signing is performed using SHA256_RSA_PKCS mechanism.
 */

"use strict";
const { graphene, withSession, usageAndExit, getPlaintext, toHex } = require("./lib/helper");

console.log("\nsign_using_rsa_sha256.js\n");

if (process.argv.length !== 3) {
  usageAndExit([
    "Usage:",
    "node sign_using_rsa_sha256.js <slot_label>",
    "",
    "Example:",
...
(async () => {
  ...
  await withSession(slotLabel, async (session) => {
    ...
    const sign = session.createSign(graphene.MechanismEnum.SHA256_RSA_PKCS, keys.privateKey);
    const signature = sign.once(Buffer.from(plaintext, "utf8"));
    console.log("Plaintext signed.");

    const verify = session.createVerify(graphene.MechanismEnum.SHA256_RSA_PKCS, keys.publicKey);
