...
# - Text will be signed using ecdsa-sha512 mechanism.
# - By default when signing mechanism is not specified, python-pkcs11 will default to ecdsa-sha512
...
print("\nsign_using_ecdsa_sha512.py\n")


# Prints the syntax for executing this code.
if len(sys.argv)!=2:
	print ("Usage:")
	print ("./sign_using_ecdsa_sha512.py <slot_label>")
	print ("\nExample:")
	print ("./sign_using_ecdsa_sha512.py SP_SKS_SEHSM3\n")
	quit()
...
try:
	...
	with p11token.open(user_pin=co_pass) as p11session: #Opens a new session and logs in as crypto officer.
		...
		signature = ecc_pri.sign(plaintext)
		print ("Plaintext signed.")

		if(ecc_pub.verify(plaintext, signature)):
