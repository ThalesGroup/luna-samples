...
# - It uses rsa-pss-sha256 mechanism for signing.
...
print("\nsign_using_rsa_pss_sha256.py\n")


# Prints the syntax for executing this code.
if len(sys.argv)!=2:
	print ("Usage:")
	print ("./sign_using_rsa_pss_sha256.py <slot_label>")
	print ("\nExample:")
	print ("./sign_using_rsa_pss_sha256.py SP_SKS_SEHSM3\n")
	quit()
...
try:
	...
	with p11token.open(user_pin=co_pass) as p11session: #Opens a new session and logs in as crypto officer.
		...
		pss_param=(Mechanism.SHA256, MGF.SHA256, 20)
		signature = rsa_pri.sign(plaintext, mechanism=Mechanism.SHA256_RSA_PKCS_PSS, mechanism_param=pss_param) # Generates signature using rsa-pss-mgf-256 mechanism.
		print ("Plaintext signed.")
		if(rsa_pub.verify(plaintext, signature, mechanism=Mechanism.SHA256_RSA_PKCS_PSS, mechanism_param=pss_param)):
