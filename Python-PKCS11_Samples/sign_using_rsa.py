...
# - Signing mechanism would default to sha512-rsa when not specified.
...
if len(sys.argv)!=2:
	print ("Usage:")
	print ("./sign_using_rsa.py <slot_label>")
	print ("\nExample:")
	print ("./sign_using_rsa.py SP_SKS_SEHSM3\n")
	quit()
...
try:
	...
	with p11token.open(user_pin=co_pass) as p11session: #Opens a new session and logs in as crypto officer.
		...
		signature = rsa_pri.sign(plaintext) # Generates signature using the default sha512-rsa mechanism.
