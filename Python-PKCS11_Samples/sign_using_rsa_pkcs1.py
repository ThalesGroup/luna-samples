...
# - This sample uses rsa with pkcs1 padding to sign data.
...
print("\nsign_using_rsa_pkcs1.py\n")


# Prints the syntax for executing this code.
if len(sys.argv)!=2:
	print ("Usage:")
	print ("./sign_using_rsa_pkcs1.py <slot_label>")
	print ("\nExample:")
	print ("./sign_using_rsa_pkcs1.py SP_SKS_SEHSM3\n")
	quit()
...
try:
	...
	with p11token.open(user_pin=co_pass) as p11session: #Opens a new session and logs in as crypto officer.
		...
		signature = rsa_pri.sign(plaintext, mechanism=Mechanism.RSA_PKCS) # Generates signature using rsa-pkcs1 mechanism.
		print ("Plaintext signed.")
		if(rsa_pub.verify(plaintext, signature, mechanism=Mechanism.RSA_PKCS)):
