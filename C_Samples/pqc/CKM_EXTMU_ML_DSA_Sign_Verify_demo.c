	...
	- It utilises the CKM_EXTMU_ML_DSA mechanism to generate signature from an externally computed mu.
	- For demonstration purpose, the sample generates an ML-DSA keypair as session objects.
	- A 64-byte dummy value to be used as external mu solely to illustrate the signing flow.
*/
...
CK_BYTE external_mu[] = { // dummpy external mu
...
{
	initParam();
	CK_MECHANISM mech = {CKM_EXTMU_ML_DSA, &additionalContext, sizeof(additionalContext)};
	checkOperation(p11Func->C_SignInit(hSession, &mech, objPrikey), "C_SignInit");
	checkOperation(p11Func->C_Sign(hSession, external_mu, sizeof(external_mu), NULL, &signatureLen), "C_Sign");
	signature = (CK_BYTE*)malloc(signatureLen);
	checkOperation(p11Func->C_Sign(hSession, external_mu, sizeof(external_mu), signature, &signatureLen), "C_Sign");
	printf("\n> Plaintext signed.\n");
...
void verifyData()
{
	...
	checkOperation(p11Func->C_Verify(hSession, external_mu, sizeof(external_mu), signature, signatureLen), "C_Verify");
	printf("\n> Signature Verified.\n");
}
