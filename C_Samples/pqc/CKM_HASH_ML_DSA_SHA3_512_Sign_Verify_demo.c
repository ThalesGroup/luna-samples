	...
	- It utilises the CKM_HASH_ML_DSA_SHA3_512 mechanism to generate signature.
	- For demonstration purpose, the sample generates an ML-DSA keypair as session objects.
	- CKM_HASH_ML_DSA_SHA3_512 is one of many Hash ML-DSA with hashing algorithm.
	- It can be used for both single-part and multi-part signing operation.
	- CK_SIGN_ADDITIONAL_CONTEXT is an optional mechanism parameter.
*/
...
CK_SIGN_ADDITIONAL_CONTEXT additionalContext;
...
void initParam()
{
	char rndContext[] = "12345678123456781234567812345678";
	additionalContext.hedgeVariant = CKH_HEDGE_REQUIRED;
	additionalContext.pContext = rndContext;
	additionalContext.ulContextLen = sizeof(rndContext)-1;
}
...
{
	initParam();
	CK_MECHANISM mech = {CKM_HASH_ML_DSA_SHA3_512, &additionalContext, sizeof(additionalContext)};
