	...
	- It utilises the CKM_ML_DSA mechanism to generate pure ML-DSA signature.
	- For demonstration purpose, the sample generates an ML-DSA keypair as session objects.
	- CKM_ML_DSA mechanism is used in this sample to sign raw bytes as-is.
	- CKM_ML_DSA is suitable for both single-part and multi-part signing operations.
*/
...
CK_SIGN_ADDITIONAL_CONTEXT optionalParam; // Optional parameter for CKM_ML_DSA mechanism.
...
// Initialises the ML-DSA signature mechanism parameter.
void initParam()
{
	char context[] = "123456781234567812345678123456781234";
	optionalParam.hedgeVariant = CKH_HEDGE_PREFERRED;
	optionalParam.pContext = context;
	optionalParam.ulContextLen = sizeof(context)-1;
}
