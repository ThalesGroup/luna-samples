        ...
        **********************************************************************************
        ...
        OBJECTIVE : This sample demonstrates how to generate some random bytes using the RNG provided by Luna HSM.
*/
...
// This function generates 32 bytes of random data.
void generateRandomBytes()
{
	int dataSize = 0;
	printf("\nEnter size of data to generate (1-65536 bytes): ");
	scanf("%d", &dataSize);
	if (dataSize > 0 && dataSize<=65536)
	{
		randomBytes = (CK_BYTE*)malloc(dataSize);
		...
		bytesToHex(randomBytes, dataSize);
	}
	else
	{
		printf("\nSize of data should be between 1-65536, exiting now...\n");
	}
}
