 ...
 * OBJECTIVE:
 * - Enable an appliance user (POST /users/{userid}/actions/enable).
 */
...
if (process.argv.length !== 4) {
  usageAndExit([
    "",
    "usage :-",
    "node user_enable.js <HSM_IP_OR_HOST> <appliance_username>",
    "",
  ]);
}
...
(async () => {
  ...
  try {
    ...
    if (res.status !== 204) {
      console.log("Enable failed. HTTP", res.status);
